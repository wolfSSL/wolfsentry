...
/*! \mainpage wolfSentry: the wolfSSL embedded firewall and IDPS
 *
 * \section descrip Description
 *
 * wolfSentry is the wolfSSL embedded firewall and IDPS (Intrusion Detection and
 * Prevention System).  It is normally built as a library that is linked in with
 * an application, which can run on bare metal or in a multiuser runtime.
 *
 * At a high level, wolfSentry is a dynamically configurable logic hub,
 * arbitrarily associating user-defined events with user-defined actions,
 * contextualized by connection attributes, tracking the evolution of the
 * client-server relationship. At a low level, wolfSentry is an embedded
 * firewall engine (both static and fully dynamic), with wildcard-capable lookup
 * of known hosts/netblocks, and unlimited extensibility through callbacks for
 * address families and action handlers, private data segments for each tracked
 * peer, and a dictionary of user-defined configuration and state nodes,
 * including freeform deep-tree JSON.
 *
 * The wolfSentry engine is dynamically configurable programmatically through an
 * API, or from a textual input file in JSON supplied to the engine. Callbacks
 * implement application-specific functionalities such as deep packet
 * inspection, orchestrations, and remote logging.
 *
 * \section docs Documentation
 *
 * Basic application integration on FreeRTOS-lwIP is documented, with usable code fragments, by [doc/freertos-lwip-app.md](freertos-lwip-app.md).
 *
 * The JSON configuration blob is documented in detail by [doc/json_configuration.md](json_configuration.md).
 *
 ...
 * ```
 *
 * generates a libwolfsentry.a that depends on only `inet_ntop()` and a handful
 * of basic string functions.  Allocator and time callbacks must then be set in
 * a `struct wolfsentry_host_platform_interface` supplied to
 * `wolfsentry_init()`.
 */
